...
function ta_frank_wolfe(ta_data; method=:bfw, max_iter_no=2000, step=:exact, log=:off, tol=1e-3)
    ...
    for k=1:max_iter_no
        ...
        if step==:exact
            ...
            # println(optk)
            # println(Optim.minimizer(optk))
            tauk = Optim.minimizer(optk)
        elseif step==:newton
            # Newton step
        ...
        if log==:on
            ...
            @printf("k=%4d, tauk=%15.10f, objective=%15f, aec=%15.10f, gap=%15.10f\n", k, tauk, objective(xk), average_excess_cost,rel_gap)
        end
